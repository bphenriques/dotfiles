...
let headers = [
  Remote-Email
  $env.GRIST_ADMIN_EMAIL
]
let output_dir = $env.OUTPUT_DIR
let workspaces = (http get --headers $headers $"($env.GRIST_URL)/api/orgs/current/workspaces")
let doc_ids = (
  $workspaces | get docs | flatten | get id
)
for doc_id in $doc_ids {
  http get --headers $headers $"($env.GRIST_URL)/api/docs/($doc_id)/download" | save --force $"($output_dir)/($doc_id).grist"
}
