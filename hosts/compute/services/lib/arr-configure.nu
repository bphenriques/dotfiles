...
def wait_ready [] {
  ...
  error make {
    msg: $"($arr_name) failed to start after 30 attempts"
  }
}
...
def ensure_root_folders [] {
  ...
  if $existing.status != 200 {
    error make {
      msg: $"Failed to get root folders: ($existing.status) - ($existing.body)"
    }
  }
  ...
  for folder in $root_folders {
    let existing_folder = (
      $existing.body | default [] | where path == $folder.path | get 0?
    )
    if $existing_folder != null {
      ...
      let payload = if $default_profile_id != null {
        {
          path: $folder.path
          defaultQualityProfileId: $default_profile_id
        }
      } else {
        {
          path: $folder.path
      ...
      if $r.status not-in [200, 201] {
        error make {
          msg: $"Failed to create root folder ($folder.path): ($r.status) - ($r.body)"
        }
      }
...
def ensure_download_client [] {
  ...
  if $existing.status != 200 {
    error make {
      msg: $"Failed to get download clients: ($existing.status) - ($existing.body)"
    }
  }
  ...
  let stale_clients = (
    $existing_clients | where implementation == "Transmission" | where name != $dc.name | where enable == true
  )
  for client in $stale_clients {
    ...
    if $r.status != 202 {
      error make {
        msg: $"Failed to disable download client ($client.name): ($r.status) - ($r.body)"
      }
    }
  }
  ...
  if $schemas.status != 200 {
    error make {
      msg: $"Failed to get download client schemas: ($schemas.status)"
    }
  }
  ...
  if $existing_client != null {
    ...
    if $r.status != 202 {
      error make {
        msg: $"Failed to update download client ($dc.name): ($r.status) - ($r.body)"
      }
    }
    ...
    if $r.status not-in [200, 201] {
      error make {
        msg: $"Failed to create download client ($dc.name): ($r.status) - ($r.body)"
      }
    }
...
def ensure_default_delay_profile [] {
  ...
  if $existing.status != 200 {
    error make {
      msg: $"Failed to get delay profiles: ($existing.status) - ($existing.body)"
    }
  }
  ...
  if $r.status not-in [200, 202] {
    error make {
      msg: $"Failed to update delay profile: ($r.status) - ($r.body)"
    }
  }
