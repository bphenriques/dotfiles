{ config, inputs, ... }:
{
  custom.homelab.services.hermes-api = {
    ...
    metadata.version = inputs.self.nixosConfigurations.hermes-vm.config.services.hermes-agent.package.version;
    metadata.homepage = "https://hermes-agent.nousresearch.com/";
    metadata.category = "Infrastructure";

    host = config.custom.fleet.microvm.hosts.hermes-vm;
    port = 8642;
