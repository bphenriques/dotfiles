def print_metadata [roms_path: path, artwork_mapping: list]: list<path> -> list<string> {
  each { |rom|
...
def process_system [system: record, roms_dir: path, artwork_mapping: list, output_dir: path] {
...
def main [config_file: path, output_dir: path] {
  ...
  for system in $config.systems {
    process_system $system $config.romsDir $config.artworkMapping $output_dir
  }
