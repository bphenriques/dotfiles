...
def find_group [name: string] {
  ...
  if $group == null { error make { msg: $"Group '($name)' not found" } }
  $group
}
...
def "main provision-users" [] {
  ...
  let group_body = { |g| { name: $g.name, friendlyName: $g.name } }
...
def "main provision-client" [] {
  ...
  if $is_group_restricted {
    ...
    if $gr.status != 200 {
      error make { msg: $"Failed to set allowed groups for ($client.name): ($gr.status) - ($gr.body)" }
    }
...
def "main guest invite" [
  ...
  let uname = if $username != null { $username } else { $email | split row "@" | get 0 }
  ...
  if $found != null { error make { msg: $"User '($uname)' already exists. Use 'reinvite' to resend the invite." } }
  ...
  let r = http post $"($base_url)/api/users" $body --headers $headers --content-type application/json --full --allow-errors
  if $r.status != 201 { error make { msg: $"Failed to create user ($uname): ($r.status) - ($r.body)" } }
  ...
  if $gr.status != 200 { error make { msg: $"Failed to assign group: ($gr.status) - ($gr.body)" } }
...
def "main list" [] {
  ...
  if ($users | is-empty) { print "No users"; return }
