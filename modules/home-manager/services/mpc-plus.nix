{ lib, pkgs, config, ... }:
let
  cfg = config.my.services.mpc-plus;
  programCfg = config.my.programs.mpc-plus;
...
in
{
  ...
  config = lib.mkIf cfg.enable {
    assertions = [ (lib.hm.assertions.assertPlatform "my.services.mpc-plus" pkgs lib.platforms.linux) ];
