# A machine with a writable store, its own boot, persistent logs and an operator: everything that is
# not a sealed microVM guest. Layered on base.nix, which carries the policy guests share.
{ lib, config, self, ... }:
let
...
in
{
  ...
  services.openssh.settings.PermitRootLogin = "prohibit-password";   # nixos-rebuild --target-host root@...
